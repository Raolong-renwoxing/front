...
ipcMain.handle('config:setApiUrl', (_, apiUrl) => {
    store.set('apiUrl', apiUrl);
    return true;
});
ipcMain.handle('config:setDefaultAssistant', (_, assistantId) => {
    store.set('defaultAssistantId', assistantId);
    return true;
});
ipcMain.handle('config:setThreadAssistant', (_, threadId, assistantId) => {
...
ipcMain.handle('config:getThreadAssistant', (_, threadId) => {
    const threadAssistants = store.store.threadAssistants || {};
    return threadAssistants[threadId] || null;
});
ipcMain.handle('config:deleteThreadAssistant', (_, threadId) => {
    const threadAssistants = store.store.threadAssistants || {};
